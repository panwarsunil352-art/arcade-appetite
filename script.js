...
const menu = [
  ...
  ["Starter Veg","Spring Roll",149,"assets/aloo-pyaz-pakoda.jpeg"],
  ...
  ["Starter Veg","Paneer Pakoda",179,"assets/aloo-pakoda.jpeg"],
  ...
  ["Starter Non-Veg","Chicken Nuggets",179,"assets/chicken-wings.jpeg"],
  ["Starter Non-Veg","Crispy Honey",239,"assets/chicken-wings.jpeg"],
  ...
  ["Rice & Noodles Non-Veg","Chilli Chicken",249,"assets/chicken-manchurian.jpeg"],
  ...
  ["Rice & Noodles Non-Veg","Sweet & Salted Crispy Chicken",239,"assets/chicken-wings.jpeg"],
  ["Rice & Noodles Non-Veg","Butter Maggi",119,""],
  ["Rice & Noodles Non-Veg","Plain Maggi",69,""],
  ["Rice & Noodles Non-Veg","Cheese Maggi",79,""],
  ["Rice & Noodles Non-Veg","Chicken Maggi",139,"assets/chicken-pakoda.jpeg"],
  ["Rice & Noodles Non-Veg","Egg Maggi",119,"assets/egg-fried-rice.jpeg"],
  ...
  ["Rice & Noodles Veg","Veg Fried Rice",169,"assets/chowmien.jpeg"],
  ["Rice & Noodles Veg","Paneer Fried Rice",189,""],
  ["Rice & Noodles Veg","Jeera Rice",129,""],
  ["Rice & Noodles Veg","Veg Manchurian",219,"assets/chilli-mushroom.jpeg"],
  ["Rice & Noodles Veg","Veg Noodles",169,"assets/hakka-noodles.jpeg"],
  ["Rice & Noodles Veg","Paneer Noodles",189,"assets/garlic-noodles.jpeg"],
  ["Tandoori Veg Starter","Malai Soya Chaap","169/249",""],
  ["Tandoori Veg Starter","Hara Bhara Kebab","159/229","assets/aloo-pakoda.jpeg"],
  ["Tandoori Veg Starter","Tandoori Paneer Tikka","229/339",""],
  ["Tandoori Veg Starter","Lesuni Paneer Tikka","229/339",""],
  ["Tandoori Veg Starter","Malai Mushroom","239/349","assets/chilli-mushroom.jpeg"],
  ["Tandoori Veg Starter","Achari Mushroom","239/349","assets/chilli-mushroom.jpeg"],
  ["Tandoori Veg Starter","Tandoori Mushroom","239/349","assets/chilli-mushroom.jpeg"],
  ["Tandoori Veg Starter","Paneer Hariyali Tikka","229/339",""],
  ["Tandoori Non-Veg Starter","Tandoori Chicken","249/449","assets/chicken-wings.jpeg"],
  ["Tandoori Non-Veg Starter","Afghani Chicken","269/459","assets/chicken-wings.jpeg"],
  ["Tandoori Non-Veg Starter","Chicken Achari Tikka","279/399","assets/chicken-pakoda.jpeg"],
  ["Tandoori Non-Veg Starter","Tandoori Fish Tikka","289/499","assets/chicken-wings.jpeg"],
  ["Tandoori Non-Veg Starter","Fish Achari Tikka","299/499","assets/chicken-wings.jpeg"],
  ["Tandoori Non-Veg Starter","Chicken Malai Tikka","279/399","assets/chicken-pakoda.jpeg"],
  ["Tandoori Non-Veg Starter","Earani Murg","279/399","assets/chicken-pakoda.jpeg"],
  ["Tandoori Non-Veg Starter","Chicken Sheekh Kebab","279/399",""],
];

const fallbackImages = [
  "assets/chilli-mushroom.jpeg","assets/chicken-wings.jpeg","assets/hakka-noodles.jpeg",
  "assets/aloo-pyaz-pakoda.jpeg","assets/french-fries.jpeg"
];
...
function money(p){ return p == null ? "Ask" : (typeof p === "number" ? "₹"+p : "₹"+p); }

function renderChips(){
  document.getElementById("chips").innerHTML = categories.map(c =>
    `<button class="chip ${c===activeCategory?"active":""}" onclick="setCategory('${c.replaceAll("'","\\'")}')">${c}</button>`
  ).join("");
}

function renderMenu(){
  const q = document.getElementById("menuSearch").value.toLowerCase().trim();
  const items = menu.filter(x => (activeCategory==="All" || x[0]===activeCategory) && (!q || x[1].toLowerCase().includes(q) || x[0].toLowerCase().includes(q)));
  const grid = document.getElementById("menuGrid");
  grid.innerHTML = items.map((x,i)=>{
    const [cat,name,price,img] = x;
    const src = img || fallbackImages[(i + name.length) % fallbackImages.length];
    const priceText = price == null ? "Ask" : (typeof price==="number" ? "₹"+price : "₹"+price);
    const addable = typeof price === "number";
    return `<article class="menu-card">
      <img class="food-img" loading="lazy" src="${src}" alt="${name}">
      <div class="menu-info">
        <div class="menu-top"><div class="menu-name">${name}</div><div class="price">${priceText}</div></div>
        <div class="cat">${cat}${cat.includes("Tandoori") && typeof price==="string" ? " • Half / Full" : ""}</div>
        <button class="add-btn" onclick="${addable ? `addToCart('${name.replaceAll("'","\\'")}',${price})` : `askPrice('${name.replaceAll("'","\\'")}')`}">${addable ? "＋ Add to order" : "Ask price on WhatsApp"}</button>
      </div>
    </article>`;
  }).join("") || `<div style="grid-column:1/-1;color:#7e8795;padding:40px 0">No matching items found.</div>`;
}

function setCategory(c){ activeCategory=c; renderChips(); renderMenu(); }
...
function renderCart(){
  ...
  document.getElementById("cartItems").innerHTML = entries.length ? entries.map(([name,v])=>`
    <div class="cart-row"><div class="cart-row-main"><div class="cart-row-name">${name}</div><div class="cart-row-price">₹${v.price} each</div></div>
    <div class="qty"><button onclick="changeQty('${name.replaceAll("'","\\'")}',-1)">−</button><b>${v.qty}</b><button onclick="changeQty('${name.replaceAll("'","\\'")}',1)">+</button></div></div>`).join("") :
    `<div class="empty-cart">Your cart is empty.<br><span>Add something delicious.</span></div>`;
}
function toggleCart(open){ document.getElementById("cart").classList.toggle("open",open); document.getElementById("cartBackdrop").classList.toggle("open",open); }
...
function sendOrder(){
  const entries=Object.entries(cart);
  if(!entries.length){ alert("Please add at least one item to your order."); return; }
  const lines=entries.map(([name,v])=>`• ${name} × ${v.qty} = ₹${v.qty*v.price}`);
  const total=entries.reduce((s,[,v])=>s+v.qty*v.price,0);
  const text=`Hi Arcade Appetite!%0A%0AI'd like to order/enquire about:%0A${encodeURIComponent(lines.join("\n"))}%0A%0AEstimated subtotal: ₹${total}%0A%0APlease confirm availability, final total and pickup/delivery details.`;
